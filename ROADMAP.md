...
- [ ] 🟡 **[P1] 视图交互与日志 (UI/UX & Logging)**
    ...
    - [x] **高级功能容器化 (Docking System)**: 初步规划完成，已升级为下方的多区域布局方案。
    - [ ] 🔴 **[P0] 多区域可折叠布局 (Multi-Zone Collapsible Layout)**
        - [ ] **三区架构**: 引入 `multi_split_view`，实现 Left/Right/Bottom 三个可独立调整尺寸的面板区域。
        - [ ] **Activity Bar**: 实现 VS Code 风格侧边导航栏，支持点击图标"展开/完全折叠"面板，只保留中间终端区。
        - [ ] **灵活配置**: 支持通过右键菜单将功能面板（如指令列表、波形）移动到不同区域 (L/R/B)。
        - [ ] **状态锁定**: 确保串口配置面板固定在 Left 区域首位，不可移动。
