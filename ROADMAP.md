...
- [ ] 🟡 **[P1] 智能自动回复系统 (Smart Auto-Reply System)**
    - [ ] **可扩展架构**: 建立统一的回复处理器接口。设计要求：新增回复模式的工作量仅限于"编写模式实现类" + "添加枚举关联"。
    - [ ] **配置管理**: 实现分层配置结构。
        - **全局设置**: 总开关、全局回复延迟 (Delay)、当前激活模式。
        - **模式设置**: 每种模式拥有独立的配置模型，统一序列化保存至 `config.json`。
    - [ ] **模式 1: 匹配回复 (Match & Reply)**:
        - 配置: 触发规则 (Hex/Ascii 包含匹配) -> 响应内容 (Hex/Ascii)。
        - 逻辑: 接收流中检测到特定特征码即触发回复。
    - [ ] **模式 2: 顺序回复 (Sequential Reply)**:
        - 配置: 预设的一组帧列表。
        - 逻辑: 每次收到数据（或特定触发）后，按顺序发送列表中的下一帧。
        - 交互: UI 需指示当前回复进度 (指针)，并支持手动重置或跳转到指定帧开始。
