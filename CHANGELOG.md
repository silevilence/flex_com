...
- 📊 **协议解析器** — 自定义帧结构配置，自动解析接收到的数据帧，直观查看各字段含义
- 🔢 **多类型数据提取** — 支持从帧中提取整数、浮点数、字节数组、ASCII 文本等多种数据类型
- 📐 **位域解析** — 精确提取单个字节内的标志位，轻松解读状态寄存器
- 🔄 **大小端自由切换** — 灵活配置数据的字节序（大端/小端），兼容各类设备协议
- ✅ **自动校验验证** — 内置 Sum8/16、XOR、CRC8/16/32 等校验算法，一键验证帧完整性
- 💾 **协议模板** — 预置 Modbus RTU 等常用协议模板，也可保存自定义配置供日后使用
- 🧮 **数值换算** — 支持比例因子和偏移量，原始数据自动转换为实际物理量并显示单位
